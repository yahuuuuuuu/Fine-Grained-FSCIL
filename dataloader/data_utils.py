...
def set_up_datasets(args):
    ...
    if args.dataset == 'mstar':
        import dataloader.mstar.mstar as Dataset
        args.base_class = 4
        args.num_classes = 8
        args.way = 2
        args.shot = 5
...
def get_transform(args):
    ...
    if args.dataset in ['mini_imagenet','mstar']:
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'mstar':
        trainset = args.Dataset.MSTAR(root=args.dataroot, train=True, index=class_index, base_sess=True,
                                       crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.MSTAR(root=args.dataroot, train=False, index=class_index)
    if args.dataset == 'StanfordDog':
...
def get_new_dataloader(args, session):
    ...
    if args.dataset == 'cub200':
        trainset = args.Dataset.CUB200(root=args.dataroot, train=True, index_path=txt_path,
                                       crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'mstar':
        trainset = args.Dataset.MSTAR(root=args.dataroot, train=True, index_path=txt_path,
                                       crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'car100':
    ...
    if args.dataset == 'cub200':
        testset = args.Dataset.CUB200(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'mstar':
        testset = args.Dataset.MSTAR(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'car100':
